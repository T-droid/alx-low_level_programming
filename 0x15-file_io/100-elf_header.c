 ...
 * @e_ident: pointer to an arry containing class
 */
void print_class(unsigned char *e_ident)
{
	printf(" Class:           ");
...
/**
 * print_version - prints version of an ELF file
 * @e_ident: a pointer to an ELF containig the version
 */
void print_version(unsigned char *e_ident)
{
...
/**
 * print_type - prints type of an ELF
 * @e_type: the type
 * @e_ident: pointer to an array  of an elf class
 */
void print_type(unsigned int e_type, unsigned char *e_ident)
{
 ...
 * @elf: the descriptor of efl file
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * @argv: stores the string of the arguments
 * Return: always 0
 */
